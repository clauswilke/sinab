...
# calculate the current width of a grob, in inch
# @param flip are width and height flipped?
# @param convert_null should null values be converted or kept as they are?
current_width <- function(grob = NULL, width = NULL, flip = FALSE, convert_null = TRUE) {
  if (is.null(width)) {
  ...
  if (is.null(grob$vp)) {
    width_in <- convert(width, 'in', TRUE)
  } else {
...
# calculate the current height of a grob, in inch
current_height <- function(grob = NULL, height = NULL, flip = FALSE, convert_null = TRUE) {
  if (is.null(height)) {
    if (isTRUE(convert_null)) {
      height <- unit(1, 'npc')
    } else {
  ...
  if (is.null(grob$vp)) {
    height_in <- convert(height, 'in', TRUE)
  } else {
    # If the grob has its own viewport then we need to push it and
    # afterwards pop it. For this to work in the general case
    # (stacked viewports, etc), we need to keep track of the depth
