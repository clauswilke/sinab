...
SEXP test_rdev_draw_text() {
  double ymax = rdev_device_height();
  Rprintf("Device height: %g", ymax);
  RenderDevice* rdev = rdev_new(ymax);
  GContext* gc = gcontext_new();

  double a, d, w_word, w_space, x;
  /* get width of a space */
  rdev_string_metrics(rdev, " ", gc, &a, &d, &w_space);

  /* draw first word */
  x = 0.5;
  rdev_draw_text(rdev, "Hello", x, 1, gc);
  
  /* advance x */
  rdev_string_metrics(rdev, "Hello", gc, &a, &d, &w_word);
  x = x + w_word + w_space;
  ...
  rdev_draw_text(rdev, "World", x, 1, gc2);
  
  /* advance x */
  rdev_string_metrics(rdev, "World", gc2, &a, &d, &w_word);
  x = x + w_word + w_space;
  
  /* draw remainder */
  gcontext_set_color(gc, "red");
  rdev_draw_text(rdev, "in red!", x, 1, gc);
